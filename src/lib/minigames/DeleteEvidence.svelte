<script lang="ts">
    ...
    function initializeParameters(){
        ...
        trashbin = [Math.floor(2*usablex/2 + offsetx), 
    ...
    function onMove(event){
        if (moving) {
            ...
            else if (event.touches[0].clientX - leftdif + spritewidth > window.innerWidth){
            ...
            else if (event.touches[0].clientY - topdif + spritewidth > window.innerHeight){
    ...
    function checkBin(){
        if ((files[currentmove][0] + 80 > trashbin[0])
...
<body on:load={initializeParameters()}>
    <section>
        <img src={trash} alt="trashbin" class="picture" style="left:{trashbin[0]}px; top:{trashbin[1]}px;">
        {#each files as file_it, i}
            {#if showfile[i]}
            <img src={file} alt="file" class="picture" style="left:{file_it[0]}px; top:{file_it[1]}px;" on:touchstart={(event)=>onStart(event, i)}>
            {/if}
        {/each}
